...
class DebugAction extends Action {
	...
	public function loadHistory(){
		...
		$query = "SELECT * FROM eclipseplusplus.epp_tracking WHERE uid = '$id'";
		$list = M()->query($query);
		for($i=0;$i<count($list);$i++){
			$list[$i]['build_repo'] = explode(',', $list[$i]['build_repo']);
			$list[$i]['build_features'] = explode(',', $list[$i]['build_features']); 
		}
		...
		if ($list === false) {
			...
			header("HTTP/1.0 200 OK");
			echo json_encode(array(page => $nowPage,status => 1, data => $list));
		}	
	}
...
//var_dump($model);
	...
	public function doBuild(){
		...
		$data['uid'] = $_SESSION[C('ADMIN_AUTH_KEY')];
